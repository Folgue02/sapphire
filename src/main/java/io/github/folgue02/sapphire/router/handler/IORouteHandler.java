...
/// insert the record in the database *([#run(HttpRequest, HttpResponse, Object)])* and then based on the returned/generated
/// data, gather some data to display in the view *([#handleOutput(HttpResponse, Object)])*.
///
/// **NOTE**: The [#runHandler(HttpRequest)] method comes from the [RouteHandler] class, but its already implemented
...
public abstract class IORouteHandler<IN, OUT> extends RouteHandler {
	...
	/// which will be passed to the [#run(HttpRequest, HttpResponse, Object)] method to be processed.
	///
